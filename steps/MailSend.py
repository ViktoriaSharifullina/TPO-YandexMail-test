...
def step_impl(context):
    wait = WebDriverWait(context.driver, 40)
    find_by_xpath(wait,
                  '/html/body/div[3]/div[2]/div[7]/div/div[3]/div/nav/div[2]/div/div/div/a').click()


@step("I fill Email field with user's email")
def step_impl(context):
    wait = WebDriverWait(context.driver, 40)
    input_whom = find_by_xpath(wait,
...
def step_impl(context):
    wait = WebDriverWait(context.driver, 40)
    find_by_xpath(wait,
                  '/html/body/div[3]/div[2]/div[10]/div/div/div[1]/div/div/div/div[1]/div[1]/div[2]/div/div[1]/div[1]/button').click()


@then("I should see a message that the letter has been sent")
def step_impl(context):
    wait = WebDriverWait(context.driver, 40)
    done = find_by_xpath(wait, '/html/body/div[16]/div/div[2]/div[1]/span')
    if done:
